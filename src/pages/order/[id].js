                ...
                <div className="flex items-center justify-center -space-x-20">
                  {data.items.map((item) => (
                    <div className="flex items-center flex-shrink-0">
                      {item.amount > 2 && (
                        <img className="relative mr-[-170px] scale-90 z-0 flex-shrink-0 opacity-80" src={`/images/thumbnails${item.coffee[0].image}`} width={200} height={200} />
                      )}
                      {item.amount > 1 && (
                        <img className="relative mr-[-170px] scale-95 z-0 flex-shrink-0 opacity-90" src={`/images/thumbnails${item.coffee[0].image}`} width={200} height={200} />
                      )}
                      <img key={item.coffee[0].id} className="test relative flex-shrink-0" src={`/images/thumbnails${item.coffee[0].image}`} width={200} height={200} />
                    </div>
                  ))}
                </div>
              </div>
                    ...
                    {data.items.map((item) => (
                      <li key={item.id} className="flex items-center justify-between border-b border-dashed border-gray-200 dark:border-white/20 last:border-none py-3 pr-3">
                        <span className="flex items-center pl-1">
                          <img key={item.coffee[0].id} className="relative flex-shrink-0" src={`/images/thumbnails${item.coffee[0].image}`} width={40} height={40} />
                          {item.amount} x {item.coffee[0].name}
                        </span>
                        <NumberFormat displayType={'text'} prefix="$" value={(item.price/100).toFixed(2)} className="opacity-75" />
                      </li>
                    ))}
                  </ul>
                  <NumberFormat displayType={'text'} prefix="$" value={(data.total/100).toFixed(2)} className="self-end font-semibold text-2xl sm:text-4xl pr-3" />
                </div>
